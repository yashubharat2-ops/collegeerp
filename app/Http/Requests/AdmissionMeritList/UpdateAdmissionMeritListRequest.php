...
class UpdateAdmissionMeritListRequest extends FormRequest
{
    public function authorize(): bool
    {
        $model = AdmissionMeritList::query()->find((int) $this->route('admission_merit_list'));
        if (! $model) {
            abort(404);
        }
    ...
    public function rules(): array
    {
        ...
        $id = $this->route('admission_merit_list');
        ...
        return [
            ...
            'code' => ['required', 'string', 'max:50', Rule::unique('admission_merit_lists')->where('college_id', $collegeId)->ignore($id)],
