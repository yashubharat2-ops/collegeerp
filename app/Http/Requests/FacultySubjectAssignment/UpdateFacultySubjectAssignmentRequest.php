...
class UpdateFacultySubjectAssignmentRequest extends FormRequest
{
    public function authorize(): bool
    {
        $model = FacultySubjectAssignment::query()->find((int) $this->route('faculty_subject_assignment'));
        if (! $model) {
            abort(404);
        }
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $v): void {
            ...
            $collegeId = app(TenantContext::class)->id();
            $ignoreId = (int) $this->route('faculty_subject_assignment');
