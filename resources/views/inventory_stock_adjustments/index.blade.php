...
<div class="panel">
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div>
            <h2 class="panel-title">Stock Adjustment</h2>
            <p class="panel-subtitle">Corrections and stock-outs. Every adjustment writes an inventory transaction with balance_after, so the ledger explains current stock.</p>
        </div>
        @if(auth()->user()?->hasPermission('inventory_stock_adjustments.create') || auth()->user()?->hasPermission('inventory_stock.adjust') || auth()->user()?->hasPermission('inventory_stock.out'))
            <a class="button" href="{{ route('inventory-stock-adjustments.create') }}">+ Record adjustment</a>
        @endif
    </div>

    <form class="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-6" method="GET" action="{{ route('inventory-stock-adjustments.index') }}">
                    ...
                    <tr class="border-b">
                        ...
                        <td class="text-xs text-slate-600">
                            ...
                            @if($movement->reference)
                                <span class="block text-slate-500">Ref: {{ $movement->reference }}</span>
                            @endif
                        </td>
                        <td class="text-slate-600">{{ $movement->creator?->name ?? '—' }}</td>
                    </tr>
                @empty
                    <tr><td class="py-4 text-slate-500" colspan="7">No stock adjustments recorded yet for this college.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
