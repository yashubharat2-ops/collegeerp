...
<div class="min-h-screen lg:flex">
    ...
    <aside class="no-print w-full bg-slate-950 text-white lg:min-h-screen lg:w-72">

        <div class="flex items-center justify-between px-6 py-6">
            <div>
                <p class="text-xs font-semibold uppercase tracking-widest text-indigo-300">
                    College ERP
                </p>
        ...
        <nav class="space-y-1 px-4 pb-6">
            ...
            <!-- Platform -->
            <div class="px-3 pb-2 pt-6 text-xs font-semibold uppercase tracking-widest text-slate-500">
                Platform
            </div>
            ...
            <!-- HR -->
            @if(
                auth()->user()?->hasPermission('faculties.view') ||
                auth()->user()?->hasPermission('departments.view') ||
                auth()->user()?->hasPermission('designations.view') ||
                auth()->user()?->hasPermission('employee_documents.view') ||
                auth()->user()?->hasPermission('staff_attendance.view') ||
                auth()->user()?->hasPermission('leave_types.view') ||
                auth()->user()?->hasPermission('leave_requests.view') ||
                auth()->user()?->hasPermission('salary_structures.view') ||
                auth()->user()?->hasPermission('salary_components.view') ||
                auth()->user()?->hasPermission('payrolls.view') ||
                auth()->user()?->hasPermission('hr_reports.view')
            )
                ...
                @if(
                    ...
                    <a class="nav-link"
                       href="{{ auth()->user()?->hasPermission('leave_requests.view') ? route('leave-requests.index') : route('leave-types.index') }}">
                ...
                @if(
                    ...
                    <a class="nav-link"
                       href="{{ auth()->user()?->hasPermission('payrolls.view') ? route('payrolls.index') : (auth()->user()?->hasPermission('salary_structures.view') ? route('salary-structures.index') : route('salary-components.index')) }}">
            ...
            <!-- Examinations -->
            @if(
                auth()->user()?->hasPermission('examinations.view') ||
                auth()->user()?->hasPermission('exam_schedules.view') ||
                auth()->user()?->hasPermission('exam_attendance.view') ||
                auth()->user()?->hasPermission('exam_marks.view') ||
                auth()->user()?->hasPermission('results.view') ||
                auth()->user()?->hasPermission('result_calculation.view') ||
                auth()->user()?->hasPermission('grade_scales.view') ||
                auth()->user()?->hasPermission('result_publishing.view') ||
                auth()->user()?->hasPermission('marksheets.view') ||
                auth()->user()?->hasPermission('grade_cards.view') ||
                auth()->user()?->hasPermission('exam_reports.view') ||
                auth()->user()?->hasPermission('student_result_history.view')
            )
            ...
            <!-- Finance -->
            @if(
                auth()->user()?->hasPermission('fee_structures.view') ||
                auth()->user()?->hasPermission('fee_categories.view') ||
                auth()->user()?->hasPermission('student_fee_assignments.view') ||
                auth()->user()?->hasPermission('fee_collections.view') ||
                auth()->user()?->hasPermission('receipts.view') ||
                auth()->user()?->hasPermission('fee_dues.view') ||
                auth()->user()?->hasPermission('fee_concessions.view') ||
                auth()->user()?->hasPermission('refunds.view') ||
                auth()->user()?->hasPermission('fee_reports.view')
            )
            ...
            <!-- Transport -->
            @if(
                auth()->user()?->hasPermission('transport_dashboard.view') ||
                auth()->user()?->hasPermission('vehicles.view') ||
                auth()->user()?->hasPermission('vehicle_documents.view') ||
                auth()->user()?->hasPermission('transport_drivers.view') ||
                auth()->user()?->hasPermission('transport_routes.view') ||
                auth()->user()?->hasPermission('student_transport_assignments.view') ||
                auth()->user()?->hasPermission('transport_fees.view') ||
                ...
                @if(auth()->user()?->hasPermission('transport_dashboard.view'))
                    <a class="nav-link" href="{{ route('transport.dashboard') }}">Transport Dashboard</a>
                @endif
                ...
                @if(auth()->user()?->hasPermission('vehicle_documents.view'))
                    <a class="nav-link" href="{{ route('vehicle-documents.index') }}">Vehicle Documents</a>
                @endif

                @if(auth()->user()?->hasPermission('transport_drivers.view'))
                    <a class="nav-link" href="{{ route('transport-drivers.index') }}">Drivers</a>
                @endif

                @if(auth()->user()?->hasPermission('transport_routes.view'))
                    <a class="nav-link" href="{{ route('transport-routes.index') }}">Routes</a>
                @endif

                @if(auth()->user()?->hasPermission('transport_routes.view'))
                    <a class="nav-link" href="{{ route('transport-stops.list') }}">Stops</a>
                @endif

                @if(auth()->user()?->hasPermission('student_transport_assignments.view'))
                    <a class="nav-link" href="{{ route('transport-assignments.index') }}">Student Transport Assignment</a>
                @endif

                @if(auth()->user()?->hasPermission('transport_fees.view'))
                    <a class="nav-link" href="{{ route('transport-fees.index') }}">Transport Fees</a>
                @endif

                @if(auth()->user()?->hasPermission('transport_reports.view'))
                    <a class="nav-link" href="{{ route('transport-reports.index') }}">Transport Reports</a>
                @endif
            ...
            <!-- Library -->
            @if(
                auth()->user()?->hasPermission('library_dashboard.view') ||
                auth()->user()?->hasPermission('books.view') ||
                auth()->user()?->hasPermission('book_categories.view') ||
                auth()->user()?->hasPermission('authors.view') ||
                auth()->user()?->hasPermission('publishers.view') ||
                auth()->user()?->hasPermission('book_copies.view') ||
                auth()->user()?->hasPermission('library_members.view') ||
                auth()->user()?->hasPermission('library_transactions.view') ||
                auth()->user()?->hasPermission('library_renewals.view') ||
                auth()->user()?->hasPermission('library_fines.view') ||
                auth()->user()?->hasPermission('library_reports.view')
            )
            ...
            <!-- Hostel -->
            @if(
                auth()->user()?->hasPermission('hostel_dashboard.view') ||
                auth()->user()?->hasPermission('hostels.view') ||
                auth()->user()?->hasPermission('hostel_buildings.view') ||
                auth()->user()?->hasPermission('hostel_rooms.view') ||
                auth()->user()?->hasPermission('hostel_beds.view') ||
                auth()->user()?->hasPermission('hostel_allocations.view') ||
                auth()->user()?->hasPermission('hostel_fees.view') ||
                auth()->user()?->hasPermission('hostel_attendance.view') ||
                auth()->user()?->hasPermission('hostel_reports.view')
            )
            ...
            <!-- Communication -->
            @if(
                auth()->user()?->hasPermission('communication_dashboard.view') ||
                auth()->user()?->hasPermission('notices.view') ||
                auth()->user()?->hasPermission('circulars.view') ||
                auth()->user()?->hasPermission('notifications.view') ||
                auth()->user()?->hasPermission('communication_templates.view') ||
                auth()->user()?->hasPermission('communication_logs.view') ||
                auth()->user()?->hasPermission('communication_tracking.view') ||
                auth()->user()?->hasPermission('communication_reports.view')
            )
            ...
            <!-- Inventory -->
            @if(
                auth()->user()?->hasPermission('inventory_dashboard.view') ||
                auth()->user()?->hasPermission('inventory_categories.view') ||
                auth()->user()?->hasPermission('inventory_items.view') ||
                auth()->user()?->hasPermission('inventory_vendors.view')
            )
        ...
        <header class="no-print flex items-center justify-between border-b border-slate-200 bg-white px-6 py-4">
            ...
            <div>
                <div class="flex items-center gap-3">
                    <p class="text-sm text-slate-500">
                        {{ app(\App\Support\Tenancy\TenantContext::class)->college()?->name ?? 'Platform' }}
                    </p>
                    ...
                    @isset($colleges)
                        @if($colleges->count() > 1)
                            <form method="POST" action="{{ route('college-context.switch') }}">
                                ...
                                <select class="rounded-lg border-slate-300 text-xs"
                                        name="college_id"
                                        onchange="this.form.submit()">

                                    @foreach($colleges as $college)
                                        <option value="{{ $college->id }}"
                                            @selected(app(\App\Support\Tenancy\TenantContext::class)->id() === $college->id)>
                                            {{ $college->name }}
                                        </option>
                                    @endforeach
            ...
            <div class="flex items-center gap-4">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="grid h-9 w-9 place-items-center rounded-full bg-indigo-100 font-bold text-indigo-700">
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button class="text-sm text-slate-500 hover:text-rose-600" type="submit">
                            Logout
                        </button>
                    </form>
