...
<div class="panel">
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div>
            ...
            <p class="panel-subtitle">{{ $vendor->name }} ({{ $vendor->code }})</p>
        </div>
    ...
    <form method="POST" action="{{ route('inventory-vendors.update', $vendor) }}" class="mt-6 grid gap-4 sm:grid-cols-2">
        @csrf
        @method('PUT')
        @include('inventory_vendors._form')
        <div class="sm:col-span-2 flex flex-wrap gap-2">
            <button class="button" type="submit">Update vendor</button>
