...
<div class="no-print mb-4 flex flex-wrap items-center justify-between gap-3">
    <div>
        <h2 class="panel-title">Marksheet — {{ $result->studentEnrollment?->student?->fullName() }}</h2>
        <p class="panel-subtitle">{{ $result->examination?->name }} · Published {{ $result->published_at?->format('M d, Y') ?? '—' }}</p>
    </div>
    <div class="flex gap-2">
        <button class="button" type="button" onclick="window.print()">Print Marksheet</button>
        <a class="button !bg-slate-200 !text-slate-700" href="{{ route('marksheets.index') }}">Back to marksheets</a>
    </div>
</div>
...
<div class="print-area mx-auto max-w-3xl">
    <div class="overflow-hidden rounded-2xl border-2 border-slate-300 bg-white shadow-sm">
        ...
        <div class="border-b-2 border-slate-800 px-6 py-5 text-center sm:px-8">
            ...
            @endif
            <p class="mt-3 text-sm font-semibold uppercase tracking-widest text-slate-700">Statement of Marks</p>
        ...
        <div class="grid gap-x-8 gap-y-2 px-6 py-5 text-sm sm:grid-cols-2 sm:px-8">
            ...
            <div class="flex justify-between gap-4 border-b border-dotted border-slate-200 py-1.5">
                ...
                <span class="text-right font-medium">{{ $result->academicYear?->name ?? '—' }}</span>
            </div>
            <div class="flex justify-between gap-4 border-b border-dotted border-slate-200 py-1.5">
        ...
        {{-- Subject-wise marks (rendered from the stored ExamResultItems) --}}
            ...
            <table class="w-full text-left text-sm">
                <thead>
                    <tr class="border-y-2 border-slate-800 text-slate-700">
                        ...
                        <th class="py-2.5 pr-2 text-right">Max Marks</th>
                        <th class="py-2.5 pr-2 text-right">Marks Obtained</th>
                ...
                <tbody>
                    @forelse($result->items as $index => $item)
                        <tr class="border-b border-slate-200">
                            ...
                            <td class="py-2.5 pr-2 font-medium">{{ $item->examSchedule?->subject?->name ?? $item->subject?->name ?? '—' }}</td>
                            <td class="py-2.5 pr-2 text-right">{{ $item->max_marks }}</td>
                            <td class="py-2.5 pr-2 text-right font-semibold">{{ $item->obtained_marks !== null ? $item->obtained_marks : '—' }}</td>
                            <td class="py-2.5 pr-2 text-center">{{ $item->grade ?? '—' }}</td>
                            <td class="py-2.5 text-center">{{ ucfirst($item->status) }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td class="py-6 text-center text-slate-500" colspan="6">No subject results were calculated.</td>
                        </tr>
                    @endforelse
                </tbody>
                <tfoot>
                    <tr class="border-y-2 border-slate-800 font-semibold">
                        <td class="py-2.5 pr-2" colspan="2">Total</td>
                        <td class="py-2.5 pr-2 text-right">{{ $result->total_max_marks }}</td>
                        <td class="py-2.5 pr-2 text-right">{{ $result->total_obtained_marks !== null ? $result->total_obtained_marks : '—' }}</td>
                        <td class="py-2.5 pr-2 text-center" colspan="2"></td>
                    </tr>
                </tfoot>
            </table>
        </div>

        {{-- Overall outcome (stored values only — nothing is recalculated here) --}}
        <div class="grid grid-cols-2 gap-3 px-6 py-4 text-center sm:grid-cols-4 sm:px-8">
            <div class="rounded-xl border border-slate-200 bg-slate-50 px-2 py-3">
                <p class="text-[11px] uppercase tracking-wide text-slate-500">Percentage</p>
                <p class="mt-1 text-base font-bold text-slate-900">{{ $result->percentage !== null ? $result->percentage.'%' : '—' }}</p>
            </div>
            <div class="rounded-xl border border-slate-200 bg-slate-50 px-2 py-3">
        ...
        <div class="flex items-end justify-between gap-6 border-t border-slate-200 px-6 py-5 text-xs text-slate-500 sm:px-8">
            <div>
                ...
                <p class="mt-2 max-w-xs">This marksheet is generated from the published examination result. In case of any discrepancy, the published result record prevails.</p>
            </div>
    ...
    <p class="no-print mt-4 text-xs text-slate-500">
        The marksheet is read from the published result record. Use your browser's print dialog to print it or save it as a PDF.
    </p>
</div>
@endsection
