{{--
    Transport fee collection — records through the EXISTING Finance payment
    rows (FeeCollectionService::collectTransportFee). The payment number, the
    outstanding cap and the collector are all server-controlled.
--}}
@php($token = (string) \Illuminate\Support\Str::uuid())
<form method="POST" action="{{ route('transport-fees.collect', $feeAssignment->id) }}" class="grid gap-3 rounded-lg border border-slate-200 bg-slate-50 p-4 sm:grid-cols-2">
    @csrf
    <input type="hidden" name="submission_token" value="{{ $token }}">
    <p class="text-sm font-semibold sm:col-span-2">Record payment ({{ $paymentModes ? 'existing Finance receipt flow' : '' }})</p>
