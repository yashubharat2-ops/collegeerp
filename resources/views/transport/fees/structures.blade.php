@extends('layouts.app')
@section('title', 'Transport Fee Structures')
@section('content')
<div class="panel">
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div>
            <h2 class="panel-title">Transport Fee Structures</h2>
            <p class="panel-subtitle">Transport-side pricing master (transport fee category). Amounts are configured per college — nothing is hard-coded and no money is collected here.</p>
        </div>
        <div class="flex gap-2">
            <a class="button !bg-slate-200 !text-slate-700" href="{{ route('transport-fees.index') }}">Transport fees</a>
            @can('create', App\Models\TransportFeeStructure::class)
                <a class="button" href="{{ route('transport-fee-structures.create') }}">+ Add structure</a>
            @endcan
        </div>
    </div>

    <form method="GET" action="{{ route('transport-fee-structures.index') }}" class="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <div class="flex gap-2">
            ...
            @if(array_filter($selected))
                <a class="button !bg-slate-200 !text-slate-700" href="{{ route('transport-fee-structures.index') }}">Clear</a>
            @endif
        </div>
    </form>
            ...
            <tbody class="divide-y">
                @forelse($structures as $structure)
                    <tr>
                        ...
                        <td class="whitespace-nowrap px-3 py-3">{{ $structure->transportRoute?->name ?? 'All routes' }}</td>
                        <td class="whitespace-nowrap px-3 py-3">{{ $structure->transportStop?->name ?? 'All stops' }}</td>
                        ...
                        <td class="px-3 py-3">
                            <div class="flex gap-2">
                                @can('update', $structure)<a class="button" href="{{ route('transport-fee-structures.edit', $structure->id) }}">Edit</a>@endcan
                                @can('delete', $structure)
                                    <form method="POST" action="{{ route('transport-fee-structures.destroy', $structure->id) }}" onsubmit="return confirm('Delete this transport fee structure? In-use structures are rejected.')">@csrf @method('DELETE')<button class="button !bg-red-600">Delete</button></form>
                                @endcan
                            </div>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="9" class="px-3 py-8 text-center text-slate-500">No transport fee structures found.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
