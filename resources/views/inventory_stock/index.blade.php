...
<div class="panel">
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div>
            ...
            <p class="panel-subtitle">The ledger behind every on-hand quantity for this college. Movements are append-only — a correction is a new movement, never an edit.</p>
        </div>
        @if(auth()->user()?->hasPermission('inventory_stock.in') || auth()->user()?->hasPermission('inventory_stock.out') || auth()->user()?->hasPermission('inventory_stock.adjust'))
            <a class="button" href="{{ route('inventory-stock.create') }}">+ Record movement</a>
        @endif
    </div>

    <form class="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-6" method="GET" action="{{ route('inventory-stock.index') }}">
                    ...
                    <tr class="border-b">
                        ...
                        <td class="text-xs text-slate-600">
                            ...
                            @if($movement->purchase_order_id)
                                <a class="block text-indigo-700" href="{{ route('inventory-purchase-orders.show', $movement->purchase_order_id) }}">Purchase order #{{ $movement->purchase_order_id }}</a>
                            @endif
                        </td>
                        <td class="text-slate-600">{{ $movement->creator?->name ?? '—' }}</td>
                    </tr>
                @empty
                    <tr><td class="py-4 text-slate-500" colspan="7">No stock movements recorded yet for this college.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
