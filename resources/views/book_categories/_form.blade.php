{{--
    Book category form (create + edit).

    XSS safety: every value is echoed through Blade's {{ }} escaping.
--}}
<div>
    <label class="label" for="name">Name</label>
    <input class="input" id="name" name="name" type="text" value="{{ old('name', $category->name ?? '') }}" required maxlength="255">
    <p class="mt-1 text-xs text-rose-600">@error('name'){{ $message }}@enderror</p>
</div>
<div>
    <label class="label" for="code">Code</label>
    <input class="input" id="code" name="code" type="text" value="{{ old('code', $category->code ?? '') }}" required maxlength="50" placeholder="e.g. REF, FIC, JRN">
...
<div>
    ...
    <p class="mt-1 text-xs text-slate-500">Inactive categories stay on the books that already use them and are flagged in the book form.</p>
